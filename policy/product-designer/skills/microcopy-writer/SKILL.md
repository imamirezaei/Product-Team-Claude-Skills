...
Ask the designer to specify:
- What element needs copy (button, error message, empty state, tooltip, dialog, etc.)
- The context (what screen, what user action triggered this state)
- The tone target (from CLAUDE.md — product voice description)
...
- Is appropriate for the screen context
...
For new copy: present ranked variants with brief rationale for each.
For copy review: present flagged issues and suggested improvements.
...
Context: [screen, user action, state]
